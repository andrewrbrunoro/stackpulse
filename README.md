...
Execute `./setup.sh` novamente para atualizar. A compilação ocorre antes da substituição do binário; uma falha no build mantém a versão anterior. Se já existir um comando diferente ou alterado no destino, o instalador informa o conflito.
...
Cada envio inicia **uma execução independente**, no mesmo projeto e com o perfil escolhido. As conversas são salvas automaticamente em `.stackpulse/sessions/<id>.jsonl`, dentro da pasta autorizada, e podem ser reabertas com `/sessions`, inclusive após reiniciar o aplicativo ou mover a pasta com seu diretório `.stackpulse`. O histórico não é enviado automaticamente como contexto ao provider. Métricas e feedback continuam no SQLite. Abrir a interface ou escolher um perfil não chama uma LLM.
