# Referência de uso do StackPulse

Instalação, comandos, configurações e interpretação das métricas. Para conhecer a finalidade do projeto, veja o [README](../README.md); para navegar pelo chat, consulte o [guia da interface](terminal-interface.md).

Os comandos com `./setup.sh` ou `cargo` pressupõem a pasta dos fontes do StackPulse.
...
![Seleção única dos plugins durante a instalação do StackPulse](assets/setup-plugins.png)
...
![Tela de conclusão do instalador em ambiente temporário](assets/installer-complete.png)
...
![Seletor de perfis no terminal](assets/terminal-profile-picker.png)
...
![Aba fixa de Configurações do chat](assets/terminal-settings.png)
...
![Abas abaixo dos widgets, ordenadas e com títulos personalizados](assets/terminal-tabs.png)
...
![Widgets de consumo e avaliação da conversa em ambiente simulado](assets/terminal-widgets.png)
...
![Detalhes e orientação de um subagente](assets/terminal-agent-detail.png)

As execuções Codex iniciadas pela UI usam um **app-server privado por pedido**, com o login e as permissões da execução. As orientações seguem sempre pelo orquestrador, responsável por encaminhá-las ao agente escolhido. **Orquestrador notificado** e **Aguardando repasse** são estados intermediários; a confirmação exige um evento nativo correlacionado ao destino e à orientação enviada. O redirecionamento exige confirmação da interrupção e do repasse. Nos demais adaptadores, essas intervenções aparecem como indisponíveis. O histórico do chat salva o último snapshot de atividade e as intervenções por pedido. Veja [os controles e seus limites](terminal-interface.md#acompanhar-os-subagentes).
...
O chat requer 64×20 e respeita `NO_COLOR`. Veja o [guia da interface](terminal-interface.md) para os atalhos, a seleção de equipes e os comandos administrativos.
...
![Painel de terminal com dados fictícios de demonstração](assets/terminal-workspace.png)
...
![Seleção do CLI no setup de terminal](assets/setup-terminal.png)
...
![Pacotes opcionais no setup: memória marcada e monitor de consumo desmarcado](assets/setup-optionals.png)
...
![Resultado visual do setup em ambiente temporário](assets/setup-complete.png)
